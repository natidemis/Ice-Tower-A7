...
Audio.prototype.asyncLoad = function(src, asyncCallback) {
    ...
    //TODO:async callback föllin keyra ekki
...
//
function audioPreload(requiredAudios,
                       loadedAudios,
                       completionCallback) {

    var numAudiosRequired,
        numAudiosHandled = 0,
        currentName,
        currentAudio,
        preloadHandler;
    ...
    numAudiosRequired = Object.keys(requiredAudios).length;
    ...
    preloadHandler = function () {
        ...
        if (numAudiosHandled === numAudiosRequired) {
            console.log("all preload images handled");
            console.log("loadedAudios=", loadedAudios);
            console.log("");
            console.log("performing completion callback");

            completionCallback();

            console.log("audio completion callback done");
            console.log("");
        }
    };
    ...
    //
    for (currentName in requiredAudios) {
        ...
        if (requiredAudios.hasOwnProperty(currentName)) {
            
            console.log("preloading audio", currentName);
            currentAudio = new Audio(currentName);
            currentAudio.name = currentName;
            console.log(currentAudio)
            currentAudio.asyncLoad(requiredAudios[currentName], preloadHandler);
        }
    }
}
